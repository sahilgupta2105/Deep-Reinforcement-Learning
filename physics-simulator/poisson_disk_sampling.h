...
public:
  ...
  static constexpr unsigned int kDims = std::tuple_size< IndexType >::value;
  ...
  */
  template <typename FloatT1>
  typename IndexType::value_type AxisIndex(
    const std::size_t i,
    const FloatT1 pos) const
  {
...
struct VecTraits<std::array<FloatT, N>>
{
  ...
  static constexpr unsigned int kSize = std::tuple_size<std::array<FloatT, N>>::value;
